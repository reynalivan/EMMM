...
class ColoredFormatter(logging.Formatter):
    ...
    def format(self, record):
        ...
        colored_level = f"{level_color}{log_level_name:<8}{LogColors.RESET}"  # 8-character padding
        ...
        # Combine all parts
        log_entry = f"{colored_time} | {colored_level} | {colored_location} - {colored_message}"
...
def setup_logger():
    ...
    console_handler.setLevel(
        logging.DEBUG)  # Process all messages from DEBUG level up
    # Use our colored formatter, adjust date format if needed
    console_formatter = ColoredFormatter(
        datefmt="%B %d, %Y > %H:%M:%S")  # Similar to loguru format
...
if __name__ == "__main__":
    ...
    if len(logger.handlers) > 1 and isinstance(logger.handlers[1],
                                               logging.FileHandler):
        log_file_path = logger.handlers[1].baseFilename
        logger.info(f"Logger setup complete. Log file: {log_file_path}")
    else:
        logger.info(
            "Logger setup complete. File handler not found as expected.")
