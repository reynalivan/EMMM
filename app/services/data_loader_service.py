...
class DataLoaderService(QObject):
    ...
    folderItemsReady = pyqtSignal(str,
                                  list)  # parent_path, list[FolderItemModel]
    ...
    def get_object_items_async(self, game_path: str) -> None:
        ...
        worker.signals.result.connect(lambda items, path=game_path: self.
                                      objectItemsReady.emit(path, items))
    ...
    def _load_object_items_task(self, game_path: str) -> list[ObjectItemModel]:
        ...
        if not os.path.exists(game_path) or not os.path.isdir(game_path):
            ...
            raise FileNotFoundError(
                f"Path not found or not a directory: {game_path}")
        ...
        for folder_name in os.listdir(game_path):
            ...
            try:
                ...
                status = not folder_name.lower().startswith(
                    DISABLED_PREFIX.lower())
                model = ObjectItemModel(path=folder_path,
                                        folder_name=folder_name,
                                        properties=properties,
                                        status=status)
                result.append(model)
            except Exception as e:
                logger.warning(f"[ObjectLoadTask] Failed on {folder_name}: {e}",
                               exc_info=True)
                continue
        logger.debug(
            f"Task: Loaded {len(result)} object items from {game_path}")
        return result
    ...
    def get_folder_items_async(self, parent_path: str) -> None:
        ...
        worker.signals.result.connect(lambda items, path=parent_path: self.
                                      folderItemsReady.emit(path, items))
        worker.signals.error.connect(
            lambda err_info, p=parent_path: self.errorOccurred.emit(
                f"Folder Loader ({os.path.basename(p)})", str(err_info[1])))
        QThreadPool.globalInstance().start(worker)

    def _load_folder_items_task(self,
                                parent_path: str) -> list[FolderItemModel]:
        ...
        for name in os.listdir(parent_path):
            ...
            try:
                ...
                model = FolderItemModel(path=abs_path,
                                        folder_name=name,
                                        info=info,
                                        status=not is_disabled)
                items.append(model)
            except Exception as e:
                logger.warning(f"[FolderLoadTask] Failed on {name}: {e}",
                               exc_info=True)
                continue
        logger.debug(
            f"Task: Loaded {len(items)} folder items from {parent_path}")
        return items
    ...
    def get_ini_files_async(self, parent_path: str) -> None:
        ...
        worker.signals.result.connect(lambda ini_files, path=parent_path: self.
                                      iniFilesReady.emit(path, ini_files))
        # --- End Modification ---
        worker.signals.error.connect(
            lambda err_info, p=parent_path: self.errorOccurred.emit(
    ...
    def _load_ini_files_task(self, parent_path: str) -> list[str]:
        ...
        try:
            ...
            logger.error(f"Task: Error listing INI files in {parent_path}: {e}",
                         exc_info=True)
    ...
    # TODO: Implement refresh_items_async and _refresh_items_task if needed
